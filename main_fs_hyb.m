...
%   '2RW_HYB'  - hybrid: intercept TV separated + 2RW bilinear on lags
%   'RWAR_HYB' - hybrid: intercept TV separated + RWAR bilinear on lags
rr_variant = 'RWAR_HYB'; % '2RW' | 'RWAR' | '2RW_HYB' | 'RWAR_HYB'
ar_factor  = 'A';        % 'A' | 'B'  -- which bilinear factor is AR(1) (used iff variant has 'RWAR')
rho_ar     = 0.99;       % AR(1) persistence (used iff variant has 'RWAR')
...
for R = 1:R_max
    ...
    is_hybrid = endsWith(upper(rr_variant), '_HYB');
    switch upper(rr_variant)
        ...
        case '2RW_HYB'
            priors.ar_factor = 'none';    % bilinear factors both RW
            draws_r = MCMC_RR_HYB_TVPVAR(Y, P, R, priors, opts);
        case 'RWAR_HYB'
            priors.ar_factor = ar_factor; % one bilinear factor AR(1)
            priors.rho       = rho_ar;
            draws_r = MCMC_RR_HYB_TVPVAR(Y, P, R, priors, opts);
        otherwise
            error('main_fs:badVariant', ...
                  'rr_variant must be ''2RW'', ''RWAR'', ''2RW_HYB'' or ''RWAR_HYB'' (got ''%s'').', ...
                  rr_variant);
    end

    IFs_r = compute_IFs(draws_r);
    if is_hybrid
        IFs_r.Phi = compute_RR_HYB_Phi_IFs(draws_r);
        Phi_q_r   = reconstruct_RR_HYB_quantiles(draws_r, qoi);
        % HYB extra fields: keep s_0 and theta_0 (small) in saved draws.
