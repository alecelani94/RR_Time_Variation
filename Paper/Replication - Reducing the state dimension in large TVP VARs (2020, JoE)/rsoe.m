...
force_stat = 1;  % force the time-invariant part to be stationary
vecm_form = 1;   % estimate using a pseudo-VECM form (i.e. VECM with full rank Pi)
th0_prior = 2;   % type of prior for theta0:
                 % 0 - baseline normal prior (no shrinkage)
                 % 1 - lasso shrinkage prior
                 % 2 - SSVS shrinkage prior
th0_minn = 1;    % set 1 to add minnesota prior shrinkage (can be combined with SSVS,
                 % but NOT compatible with LASSO -- make sure one or the other is off)
sharedvol = 1;   % set 0 for reduced errors in coeff and standard SV;
                 % set 1 for reduced errors in coeff and independently volatilities;
                 % set 2 to allow error sharing between volatilies and coeff;
                 % set 3 to allow sharing using an alternative volatilies process
...
rmaxx = 06; % maximum number of err sources (in coefficients)
rmaxh = 03; % maximum number of err sources (in volatilities) for sharedvol = 1;
            % note: to specify rmaxh = n, set rmaxh = -1 here (it will be overriden later)
...
Ydata = load( 'newsshocks03.csv' );
% Ydata = Ydata( :, 1:6 );   % small data set
% Ydata = Ydata( :, 1:8 );   % medium data set
Ydata = Ydata( :, 1:15 );       % large data set
% Ydata = Ydata( :, 1:25 );       % huge data set
...
if vecm_form
    ...
    dscale_lev = std( Ylevel, 1 ); dcenter_lev = zeros( 1, n ); %mean( Ylevel, 1 );
    Ylevel = ( Ylevel - repmat( dcenter_lev, T, 1 ) ) ./ repmat( dscale_lev, T, 1 );
...
if th0_minn
    ...
    if vecm_form
        VTh00_1 = [repmat( 1, n, n + 1 ) ...             % correlations and intercept
                   repmat( 10 ^ 2 / ( 2 * n ), n, n )];  % Pi coefficients
        [~, VTh00_2] = minnprior( shortY, p );           % AR coefficients
        VTh00 = [VTh00_1 reshape( VTh00_2, n, n * p )];
        iVth00 = diag( 1 ./ VTh00( mat2vec ) );
        th00 = zeros( nk, 1 );
    else
        VTh00_1 = repmat( 1, n, n + 1 );                        % correlations and intercept
        [Th00_2, VTh00_2] = minnprior( shortY, p );             % AR coefficients
        VTh00 = [VTh00_1 reshape( VTh00_2, n, n * p )];
        iVTh00_3 = blkdiag( sparse( n ^ 2 + n, n ^ 2 + n ), ... % Prior on sum of coefficients
                 ( 2 * n / 10 ^ 2 ) * repmat( speye( n ^ 2 ), p, p ) );
        iVth00 = diag( 1 ./ VTh00( mat2vec ) ) + iVTh00_3( mat2vec, mat2vec );
        Th00 = [zeros( n, k - n * ( p - 1 ) ) reshape( Th00_2, n, n * p )];
        th00 = Th00( mat2vec );
    end
else
    % just do standard normal priors
    th00 = zeros( nk, 1 ); iVth00 = sparse( 1:nk, 1:nk, 0.1 );
end
lam20( 1 ) = 2; lam20( 2 ) = 0.1; % for the lasso priors
c0 = 1e-2; q0 = 0.5; iVth00_0 = diag( iVth00 ); % for the SSVS priors
...
%         iVb0 = sparse( 1:nnz( freeb ), 1:nnz( freeb ), [iVth00; iVax0( freeax )] );
        iVb0 = blkdiag( iVth00, sparse( 1:nnz( freeax ), 1:nnz( freeax ), iVax0( freeax ) ) );
...
%         Ax = reshape( ax, nk, rx );
        iVb11 = iVb( 1:nk, 1:nk );          iVb12 = iVb( 1:nk, nk + 1:end );
        iVb21 = iVb( nk + 1:end, 1:nk );    iVb22 = iVb( nk + 1:end, nk + 1:end );
        b_hat1 = b_hat( 1:nk );
        b_hat2 = b_hat( nk + 1:end );
        
        b2 = b_hat2 + chol( iVb22 - iVb21 * ( iVb11 \ iVb12 ) ) \ randn( nnz( freeax ), 1 );
        ax = zeros( nk, rx );
        ax( freeax ) = b2;
        Ax = reshape( ax, nk, rx );

        theta0_hat = b_hat1 - iVb11 \ ( iVb12 * ( b2 - b_hat2 ) );
        CiVb11 = chol( iVb11 );
        stationary = false;
        count = 0;
        while ~stationary && count < 1000
            theta0 = theta0_hat + CiVb11 \ randn( nk, 1 );
            if isim > max( 0.01 * burnin, 100 ) && force_stat
                Theta0 = zeros( n, n + k );
                Theta0( freeth ) = theta0( vec2mat );
            	iA = Sdiff .* Theta0( :, 1:n ) + eye( n );
                
                if vecm_form
                    Pi = Slev .* Theta0( :, n + 2:2 * n + 1 );
                    G = repmat( Sdiff, 1, p ) .* Theta0( :, 2 * n + 2:end );
                	bigB = [iA \ [Pi G] * R1 + R0; [eye( n * p ) zeros( n * p, n )]];
                else
                    G = repmat( Sdiff, 1, p ) .* Theta0( :, n + 2:end );
                    bigB = [iA \ G; [eye( n * ( p - 1 ) ) zeros( n * ( p - 1 ), n )]];
                end
                stationary = all( abs( eig( bigB ) ) <= 1 );
            else
                stationary = true;
            end
            count = count + 1;
...
%         iVth0 = iVth00 + bigXibigSig * bigX;
%         th0_hat = iVth0 \ ( iVth00 * th00 + bigXibigSig * ( y - W * ax ) );
%         theta0 = th0_hat + chol( iVth0 ) \ randn( nk, 1 );
        
        if th0_prior == 1
            % do lasso on iVth00
            lam2_th = gamrnd( lam20( 1 ) + nk, 1 / ( lam20( 2 ) + sum( 1 ./ diag( iVth00 ) ) / 2 ) );
            iVth00 = spdiags( igrand( sqrt( lam2_th ) ./ abs( theta0 - th00 ), lam2_th ), 0, iVth00 );
        elseif th0_prior == 2
            % do SSVS on iVth00
            diff_th = theta0 - th00;
            ...
            iVth00 = spdiags( 1 ./ ( ( c0 + ( rand( nk, 1 ) < pr( :, 2 ) ) * ( 1 - c0 ) ) ./ iVth00_0 ), 0, iVth00 );
        end
        ...
        if sharedvol < 2
            ...
            if rx > 0
                H = spdiags( -ones( T * rx, 1 ), -rx, speye( T * rx ) );
                [gam_hat, iVgam] = gam_mode( diffy, XA, Ah, h0, H );
...
mAh = mean( s_Ahid, 3 );
rsoe_DIC;
iffsDy = ineff_factor( s_Dy' );
p_D3, DIC3, seDIC3

%% Macroeconimc Application
valid = zeros( svsims, 1 );
for isim = 1:svsims
    Theta0 = zeros( n, n + k );
    Theta0( freeth ) = s_theta0( vec2mat, isim );
    iA = Sdiff .* Theta0( :, 1:n ) + eye( n );
    Pi = Slev .* Theta0( :, n + 2:2 * n + 1 );
    G = repmat( Sdiff, 1, p ) .* Theta0( :, 2 * n + 2:end );
    bigB = [iA \ [Pi G] * R1 + R0; [eye( n * p ) zeros( n * p, n )]];
    valid( isim ) = max( abs( eig( bigB ) ) );
end
recover_struct_params
make_irfs_fevds_plots
freeIR = true( n, n, 1 + irf_hrz, T - irf_hrz, svsims );
freeIR( 1, 2:end, 1, :, : ) = false;
iffsIR = ineff_factor( reshape( s_IR( freeIR ), nnz( freeIR ) / svsims, svsims ) );
%%
% IR = irf( 20, [45 87 T - 20], [1 2 4 11], [1 2 4 11], s_theta0, s_Ax, s_gammax, s_h0, s_Ah, s_gammah, T, n, k, p );
% % need to adjust for these original standardizations of the data
% d0center = [0.009179929	0.00044587	0.1059375	-0.020584496	0.003476487	4.96E-06	3.31E-05	0.009400456	0.010391251	0.0125	7.323998392	0.000651726	0.008280926	0.000341754	5.57E-05	0.020579103	0.0575	-0.002929436	0.011535599	0.006154575	55.9796875	57.5265625	0.0065556];
% d0center = d0center( [1 2 8 10 11 12 14 19 20 21 3 5 6 16 17] );
% d0scale = [0.009611074	0.002980611	0.829133054	0.590959952	0.056611006	0.004927057	0.007476468	0.008039631	0.019190352	0.3125	0.207765058	0.009298284	0.006573622	0.010450898	0.005608216	0.10747878	0.274231882	0.020572735	0.04388689	0.005171625	6.747272127	7.274940768	0.010207773];
% d0scale = d0scale( [1 2 8 10 11 12 14 19 20 21 3 5 6 16 17] );
% 
% gdp_ffr = squeeze( IR( 1, 4, :, :, : ) ); gdp_ffr = gdp_ffr * dscale( 1 ) * d0scale( 1 ) / dscale( 11 ) / d0scale( 11 );
% inf_ffr = squeeze( IR( 2, 4, :, :, : ) ); inf_ffr = inf_ffr * dscale( 2 ) * d0scale( 2 ) / dscale( 11 ) / d0scale( 11 );
% ump_ffr = squeeze( IR( 3, 4, :, :, : ) ); ump_ffr = ump_ffr * dscale( 4 ) * d0scale( 4 ) / dscale( 11 ) / d0scale( 11 );
% 
% ffr_gdp = squeeze( IR( 4, 1, :, :, : ) ); ffr_gdp = ffr_gdp * dscale( 11 ) * d0scale( 11 ) / dscale( 1 ) / d0scale( 1 );
% ffr_inf = squeeze( IR( 4, 2, :, :, : ) ); ffr_inf = ffr_inf * dscale( 11 ) * d0scale( 11 ) / dscale( 2 ) / d0scale( 2 );
% ffr_ump = squeeze( IR( 4, 3, :, :, : ) ); ffr_ump = ffr_ump * dscale( 11 ) * d0scale( 11 ) / dscale( 4 ) / d0scale( 4 );
% 
% ffr_ffr = squeeze( IR( 4, 4, :, :, : ) );
% 
% % undo the "sationarity" transformations
% cgdp_ffr = cumsum( gdp_ffr, 1 );
% cinf_ffr = cumsum( inf_ffr, 1 );
% cump_ffr = cumsum( ump_ffr, 1 );
% 
% % economic variable responses to non-systematic monetary policy in 1975Q1
% set( 0, 'DefaultAxesColorOrder', [0 0 0], 'DefaultAxesLineStyleOrder', '--|-|--', 'DefaultAxesPosition', [0.1, 0.05, 0.875, 0.925] );
% figure, plot( 0:20, squeeze( prctile( gdp_ffr( 1:21, 45, : ), [16, 50, 84], 3 ) ), 'LineWidth', 2 );
% figure, plot( 0:20, squeeze( prctile( inf_ffr( 1:21, 45, : ), [16, 50, 84], 3 ) ), 'LineWidth', 2 );
% figure, plot( 0:20, squeeze( prctile( ump_ffr( 1:21, 45, : ), [16, 50, 84], 3 ) ), 'LineWidth', 2 );
% 
% % economic variable responses to non-systematic monetary policy in 1981Q3
% figure, plot( 0:20, squeeze( prctile( gdp_ffr( 1:21, 87, : ), [16, 50, 84], 3 ) ), 'LineWidth', 2 );
% figure, plot( 0:20, squeeze( prctile( inf_ffr( 1:21, 87, : ), [16, 50, 84], 3 ) ), 'LineWidth', 2 );
% figure, plot( 0:20, squeeze( prctile( ump_ffr( 1:21, 87, : ), [16, 50, 84], 3 ) ), 'LineWidth', 2 );
% 
% % economic variable responses to non-systematic monetary policy in 1998Q2
% figure, plot( 0:20, squeeze( prctile( gdp_ffr( 1:21, end, : ), [16, 50, 84], 3 ) ), 'LineWidth', 2 );
% figure, plot( 0:20, squeeze( prctile( inf_ffr( 1:21, end, : ), [16, 50, 84], 3 ) ), 'LineWidth', 2 );
% figure, plot( 0:20, squeeze( prctile( ump_ffr( 1:21, end, : ), [16, 50, 84], 3 ) ), 'LineWidth', 2 );
% 
% set( 0, 'DefaultAxesColorOrder', [0 0 0; 0.4, 0.4, 0.4; 0.7, 0.7, 0.7], 'DefaultAxesLineStyleOrder', '-|-|-', 'DefaultAxesPosition', [0.1, 0.05, 0.875, 0.925] );
% figure, plot( 0:20, squeeze( prctile( gdp_ffr( 1:21, [87 145 end], : ), 50, 3 ) ), 'LineWidth', 2 );
% figure, plot( 0:20, squeeze( prctile( inf_ffr( 1:21, [87 145 end], : ), 50, 3 ) ), 'LineWidth', 2 );

% [mtheta0, mAx, mh0, mAh, iVbx, iVbx_bx_hat, iVh0, iVh0_h0_hat, Sh, iVbh, iVbh_bh_hat] = quickmmap_lowram1( y, X, bigX, hugeX, Xones, th00, ax0, h00, ah0, iVax0, iVh00, iVah0, sh0, Sh0, s_theta0, s_Ax, s_h0, s_Ah, s_sigh, s_gammax, s_gammah, s_h, s_iVth00, s_S, k, sharedvol );
% rsoe_DIC;
% DICjkvar;
% sqrt( jkvDIC1 ), sqrt( jkvDIC2 )

%%% recycle %%%
% %             [ln_prop_rat0, ln_prop_rat], pause
%             f = @( x ) -0.5 * sum( ( H * x ) .^ 2 ) - 0.5 * sum( vec( repmat( h0, 1, T ) + Ah * reshape( x, rx, T ) ) + exp( -vec( repmat( h0, 1, T ) + Ah * reshape( x, rx, T ) ) ) .* ( diffy - XA * x ) .^ 2 );
%             minusf = @(x) -f(x);
%             gammacc = reshape( fminunc( minusf, gammac( : ) ), rx, T );
%             norm( gammacc( : ) - gam_hat ), -f( gammacc( : ) ), -f( gam_hat ), pause
%             f( gammac( : ) ) - f( gammax( : ) ), ln_prior_rat + ln_like_rat, pause
% %             [f( gammac( : ) ) - f( gam_hat ), -0.5 * ( gammac( : ) - gam_hat )' * iVgam * ( gammac( : ) - gam_hat )]
% %             [f( gammax( : ) ) - f( gam_hat ), -0.5 * ( gammax( : ) - gam_hat )' * iVgam * ( gammax( : ) - gam_hat )]
%             [f( gammac( : ) ) - f( gam_hat ), -( n + nu ) / 2 * log( nu + ( gammac( : ) - gam_hat )' * iVgam * ( gammac( : ) - gam_hat ) ) + ( n + nu ) / 2 * log( nu )]
%             [f( gammax( : ) ) - f( gam_hat ), -( n + nu ) / 2 * log( nu + ( gammax( : ) - gam_hat )' * iVgam * ( gammax( : ) - gam_hat ) ) + ( n + nu ) / 2 * log( nu )], pause
%             max( abs( grad ) )
%             -0.5 * sum( ( H * gammac( : ) ) .^ 2 ) - 0.5 * sum( hc( : ) + exp( -hc( : ) ) .* erryc .^ 2 ) - ( sum( log( diag( chol( iVgam ) ) ) ) - 0.5 * ( gammac( : ) - gam_hat )' * iVgam * ( gammac( : ) - gam_hat ) ), pause
