...
for t = 1:s
    for i = 1:nmax
        bounds = squeeze( prctile( s_IR( i, shock, 1:irf_hrz + 1, t * tstep, valid ), [16 84], 5 ) );
        ymin( i ) = min( [ymin( i ); bounds( :, 1 )] );
        ymax( i ) = max( [ymax( i ); bounds( :, 2 )] );
        
        h( t, i ) = subplot( s, nmax, ( t - 1 ) * nmax + i );
        box on; hold on;
        plot( 0:irf_hrz, squeeze( mean( s_IR( i, shock, 1:irf_hrz + 1, t * tstep, valid ), 5 ) ), 'k-', 'LineWidth', 2 );
        plot( 0:irf_hrz, bounds, 'r-', 'LineWidth', 2 );
        plot( 0:irf_hrz, zeros( irf_hrz + 1, 1 ), 'b:', 'LineWidth', 2 );
...
print( sprintf( 'irfs2nonnews_n%0.2u_rx%0.2u_rh%0.2u_sharedvol%0.1u', n, rx, rh, sharedvol ), '-depsc', '-painters' );